...
def collect_groups(node):
    if not isinstance(node, dict):                 # leaf
        return [node] if len(node) > 1 else []     # keep it only if it's a group
    return collect_groups(node[True]) + collect_groups(node[False])

groups = collect_groups(tree)
groups.sort(key=len, reverse=True)                 # biggest first
